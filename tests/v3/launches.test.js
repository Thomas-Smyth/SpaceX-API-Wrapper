...
test('It should return all launches', async () => {
  const data = await SpaceX.getAllLaunches();
  data.forEach(item => {
...
test('It should return the latest launch', async () => {
  const data = await SpaceX.getLatestLaunch();
  expect(data).toHaveProperty('upcoming', false);
});
...
test('It should return the next launch', async () => {
  const data = await SpaceX.getNextLaunch();
  expect(data).toHaveProperty('upcoming', true);
});
...
test('It should return no launches due to invalid UTC date', async () => {
  const data = await SpaceX.getAllLaunches({
    launch_date_utc: '2011-25-05T14:48:00.000Z'
  });
  expect(data).toEqual([]);
});
...
test('It should return the upcoming launches', async () => {
  const data = await SpaceX.getUpcomingLaunches();
  data.forEach(launch => {
